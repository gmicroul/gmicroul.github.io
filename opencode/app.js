...
const ISO_FILENAME = "opencode-v86-agent-final.iso";
const ISO_SIZE = 313524224;
...
function disableKeyboard(instance = emulator) {
  if (instance) instance.keyboard_set_status(false);
}
...
function initialize() {
  ...
  try {
    ...
    instance.add_listener("emulator-ready", () => {
      if (emulator !== instance) return;
      ui.loadingTitle.textContent = "正在启动 opencode 运行环境";
      ui.loadingDetail.textContent = "ISO 和 virtio 网卡已就绪";
      ui.progress.style.width = "100%";
      setControlsEnabled(true);
      disableKeyboard(instance);
    });
...
document.querySelector(".toolbar").addEventListener("pointerdown", () => {
  disableKeyboard();
});
...
ui.screenshot.addEventListener("click", () => {
  ...
  image.addEventListener("load", () => {
    ...
    link.download = `opencode-v86-${new Date().toISOString().replace(/[:.]/g, "-")}.png`;
    link.href = image.src;
    link.click();
  }, { once: true });
});
...
ui.screen.addEventListener("focus", () => {
  if (emulator) emulator.keyboard_set_status(true);
});
ui.screen.addEventListener("blur", () => disableKeyboard());
window.addEventListener("blur", () => disableKeyboard());
window.addEventListener("compositionstart", () => disableKeyboard(), true);
window.addEventListener("compositionend", () => {
  if (document.activeElement === ui.screen && emulator) {
