...
ISO 约 170 MB，不能直接提交到 GitHub 仓库。当前文件已经作为
`gmicroul/gmicroul.github.io` 的 `tinycore-browser-fixed` Release 资产上传：
...
对象的单文件上限是 100 MB。页面直接通过上面的 Release URL 按需读取 ISO。
...
`dist` 中的 ISO；GitHub Pages 模式读取 Release。页面可用 `?iso=<URL>` 指定
其他 ISO，并可用
...
提交 `index.html`、`styles.css`、`app.js`、`vendor/`、`licenses/` 以及构建
脚本。`dist/*.iso` 已由 `.gitignore` 排除，ISO 只需上传到 Release。
